...
class AntiDataset(BaseWrapperDataset):
    def __init__(
        self,
        dataset,
        labels,
    ...
    def get_ref(self, index):
        return (
            self.labels[index]
            if self.process_label is None
            else self.process_label(self.refs[index])
        )

    def __getitem__(self, index):
        item = self.dataset[index]
        item["label"] = self.get_label(index)
        item["ref"] = self.get_ref(index)
        return item
    ...
    def tomask(self, target):
        mil = []
        for tar in target:
            _ = torch.zeros(len(self.target_dict))
            _[tar.long()] = 1
            mil.append(_)
        return torch.stack(mil,dim=0)
    ...
    def collater(self, samples):
        ...
        text = [s["ref"] for s in samples if s["id"] in indices]

        ref = [s["ref"].unique() for s in samples if s["id"] in indices]
        alt_ant = [torch.Tensor([i for i in range(4,len(self.target_dict)) if i not in _ref]) for _ref in ref]
        ref, alt_ant, ann = self.tomask(ref), self.tomask(alt_ant), self.tomask(target)
...
@register_task("mix", dataclass=AudioPretrainingConfig)
class MixTask(FairseqTask):
    ...
    def load_dataset(
        ...
        if task_cfg.labels:
            ...
            ref_path = os.path.join(data_path, f"{split}.ref")
            ...
            assert len(refs) == len(labels) == len(self.datasets[split]), (
                    f"labels(or texts) length ({len(labels)}) and dataset length "
                    f"({len(self.datasets[split])}) do not match")

            process_label = LabelEncoder(self.target_dictionary)
        
            self.datasets[split] = AntiDataset(
                self.datasets[split],
                labels,
                refs,
                ...
                target_dict=self.target_dictionary,
    ...
    def build_model(self, model_cfg: FairseqDataclass):
        ...
        if self.cfg.eval_wer and self.cfg.autoregressive:
            ...
            if self.cfg.eval_wer_tokenizer:
                ...
                self.tokenizer = None
        return model
    def _inference_phone_cls(self, generator, sample, model):
        return 
