...
@register_model("wav2vec_error", dataclass=Wav2Vec2CtcConfig)
...
class Wav2VecEncoder(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrConfig, tgt_dict=None):
        ...
        self.decoder =  TextDec(emb_dim=768, tgt_dict=tgt_dict, num_layers=1, add=cfg.attn_add)
        if d != 768:
    ...
    def forward(self, source, padding_mask, tbc=True, **kwargs):
        ...
        if self.proj:
            x = self.proj(x)
        x, e, attn = self.decoder(x, k, padding_mask=text_mask)
...
class TextDec(nn.Module):
    def __init__(self, emb_dim, tgt_dict, num_layers, add):
