...
class TextConditionDataset(BaseWrapperDataset):
    def __init__(
        self,
        dataset,
        labels,
    ...
    def collater(self, samples):
        ...
        collated["iid"] = [ sample["iid"] for sample in samples]
        ...
        if self.add_to_input:
            padding_mask = torch.BoolTensor(text.shape[0],text.shape[1]+1).fill_(False)
        else:
        ...
        if self.add_to_input: # add blank at the start
            ...
            collated["text"] = torch.cat([text, eos], dim=-1).long()
...
@register_task("text_condition", dataclass=AudioPretrainingConfig)
class TextConditionTask(FairseqTask):
    """"""
    ...
    def load_dataset(
        ...
        if task_cfg.labels:
            ...
            text_path = os.path.join(data_path, f"{split}.ref")
            ...
            self.datasets[split] = TextConditionDataset(
                self.datasets[split],
                labels,
                texts,
                pad=self.target_dictionary.pad(),
                eos=self.target_dictionary.eos(),
                blank=self.target_dictionary.eos(),
                batch_targets=True,
