...
def process_predictions(
    args, hypos, sp, tgt_dict, target_tokens, res_files, speaker, id, ppg_file=None
):
    for hypo in hypos[: min(len(hypos), args.nbest)]:
        ...
        if ppg_file is not None:
            with open(ppg_file, "ab", buffering=1) as f:
                pkl.dump({id: hypo["ppg"]}, f)
...
def prepare_ppg_files(args):
    ...
    if not args.results_path:
        return None
    ppg_file_name = get_res_file("hypo.ppgs")
    if os.path.exists(ppg_file_name):
        with open(ppg_file_name, 'wb') as f:
            pass
    return ppg_file_name, os.path.join(args.results_path, 'tgt_dict.pkl')
...
def main(args, task=None, model_state=None):
    ...
    def build_generator(args):
        ...
        if w2l_decoder == "viterbi":
            from examples.speech_recognition.transducer_decoder import W2lViterbiDecoder

            return W2lViterbiDecoder(args, task.target_dictionary)
        elif w2l_decoder == "kenlm":
            from examples.speech_recognition.w2l_decoder import AttentionDecoder

            return AttentionDecoder(args, task.target_dictionary)
            ...
            from examples.speech_recognition.transducer_decoder import TransducerDecoder
    ...
    if args.w2l_decoder == "teacher":
        ppg_file, tgt_file = prepare_ppg_files(args)
